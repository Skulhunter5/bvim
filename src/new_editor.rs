...
impl NewEditor {
    pub fn new() -> Result<Self> {
        ...
        NewEditor::create(text, None)
    }

    pub fn new_with_file(path: String) -> Result<Self> {
        let text = NewEditor::load_file(&path)?;

        NewEditor::create(text, Some(path.to_string()))
    }
    ...
    pub fn run(&mut self) -> Result<()> {
        ...
        while !self.terminate {
            ...
            stdout.queue(Print(format!("Redraws: {}", self.screen.get_redraws())))?;
    ...
    fn move_to_current_position(&mut self) {
        ...
        //stdout().queue(MoveTo(self.col as u16, self.row as u16)).unwrap();
    }
